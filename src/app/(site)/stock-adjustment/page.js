...
export default function Inventory() {
    ...
    // Fetch details for all inventories
    const fetchAllInventoryDetails = async (inventoryList) => {
        const detailsArr = [];
        ...
        setInventoryDetails(detailsArr);
    };

    // Fetch details when inventories change (including on first load)
    useEffect(() => {
    ...
    const handleEdit = (id) => {
        window.open(`/stock-adjustment/modify/update/${id}`, "_blank");
    }

    const handleDelete = async () => {
        await inventoryService.deleteStockAdjustment(deleteId)
            .then(async () => {
                setModalSuccessMessage("Xóa phiếu kiểm thành công");
                setModalSuccessOpen(true);
                setModalDeleteOpen(false);
                await fetchInventories();
                setInventoryDetails([]); // Clear details
                // Refetch details for the new inventories list
                fetchAllInventoryDetails(inventories);
            })
            .catch((error) => {
                setModalFailedMessage(`Lỗi: ${error.response.data.error.message}`);
    ...
    const handleDeleteModal = (id) => {
        setDeleteId(id);
        setModalDeleteMessage("Bạn có muốn xóa phiếu kiểm này không?");
                ...
                <div className='w-auto rounded-xl h-auto bg-white px-4 py-2'>
                    ...
                    <div className='flex flex-row justify-between items-center p-4'>
                        <div className='flex flex-row items-center gap-2'>
                            ...
                            {inventory && inventory.status === 1 && <button className='rounded-xl px-4 py-2 bg-red-500 text-white' onClick={() => handleDeleteModal(inventory.adjustmentId)}>Xóa</button>}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="flex flex-col p-4">
            <div className="flex flex-row mb-2 bg-white p-4 rounded-xl mb-4 justify-between">
                ...
                <div className="flex flex-col gap-2">
                    <button className="block border background-primary text-white cursor-pointer rounded-xl w-full font-semibold h-10 rounded my-auto px-4" onClick={() => navigate("/stock-adjustment/modify/create")}>Tạo phiếu kiểm mới</button>
                </div>
            </div>
            ...
            />
            <ConfirmModal isOpen={modalDeleteOpen} message={modalDeleteMessage} onClose={() => setModalDeleteOpen(false)} onConfirm={handleDelete} onCancel={() => setModalDeleteOpen(false)} />
